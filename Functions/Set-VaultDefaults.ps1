...
#>
    ...
    param (
        ...
        [ValidateNotNullOrEmpty()]
        [String]
        $Role=$(throw "Role is mandatory, please provide a value")
    )
    ...
    process {
        ...
        if($AuthMethod -eq 'EC2' -and ( $Vault_Nonce -eq $null -or $Vault_Nonce.ToString().length -ne 32) ){
